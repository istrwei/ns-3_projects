...
struct RouteTableEntry
{
  ...
  std::string nextHopNum;
  Ipv4Address interfaceAddr;
  uint32_t cost;

  RouteTableEntry () : // default values
	  destNum (),
	  nextHopAddr (), nextHopNum (),interfaceAddr(), cost(){};
};
...
class LSRoutingProtocol : public PennRoutingProtocol
{
  public:
    ...
    void CheckNeighbors();
    void ProcessNdiscRsp (LSMessage lsMessage,Ptr<Socket> socket);
    void ProcessNdiscReq (LSMessage lsMessage);
  ...
  protected:
    ...
    void AddEntry (const Ipv4Address &neighborAddr,
                   uint32_t interface,
                   std::string &nodeNum, Ipv4Address const &interfaceAddr);

    bool Lookup (const Ipv4Address &neighborAddr) const;
    ...
    bool Lookup (Ipv4Address const &dest,
  ...
  private:
    ...
    std::map<std::string, uint32_t> lspCheck;                   // key is node number. value is the timestamp of the message
    std::map<Ipv4Address, std::map<Ipv4Address, NeighborTableEntry> > lspTableList;         // key is node number, value is this node's neighbor table
    std::map<Ipv4Address, std::map<Ipv4Address, NeighborTableEntry> > old_lspTableList;
    bool needFlood;
    bool neighborChanged;
    std::map<Ipv4Address, RouteTableEntry> m_RouteTable;
