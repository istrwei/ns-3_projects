...
// neibor discovery
    ...
    void CheckNeighbors();
    void ProcessNdiscRsp (DVMessage dvMessage,Ptr<Socket> socket);
    void ProcessNdiscReq (DVMessage dvMessage);
  ...
  protected:
    ...
    struct NeighborTableEntry
    {
      ...
      NeighborTableEntry () : // default values
    	  neighborAddr (),
        interface (0), nodeNum (),counter(0), flag(false), cost(1){};
    };

    void AddEntry (const Ipv4Address &neighborAddr,
                   uint32_t interface,
                   std::string &nodeNum,Ipv4Address localAddr );

    bool Lookup (const Ipv4Address &neighborAddr,
				  NeighborTableEntry &outEntry) const;

    void DumpTable ();

    void BrocastDV();
    void ProcessDV (DVMessage dvMessage);
    void DvCompute();
    bool Lookup (Ipv4Address const &dest, DvEntry &outEntry) const;
  ...
  private:
    ...
    Timer m_dvTimer;
    Timer m_dvBrocastTimer;



    std::map<Ipv4Address, DvEntry> m_DvTable;
    std::map<Ipv4Address, std::map<Ipv4Address, DvEntry> > m_AllDvTabel;
